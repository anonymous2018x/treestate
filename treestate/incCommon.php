	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'products' => array('Products', '', 'resources/table_icons/building.png', 'None'),
			'locations' => array('Locations', '', 'resources/table_icons/building.png', 'None'),
			'patners' => array('Patners', '', 'resources/table_icons/administrator.png', 'None'),
			'orders' => array('Orders', '', 'resources/table_icons/administrator.png', 'None')
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'products' => "`products`.`id` as 'id', `products`.`name` as 'name', `products`.`cost` as 'cost', `products`.`weight` as 'weight', `products`.`quantity` as 'quantity', `products`.`category` as 'category', `products`.`brand` as 'brand', `products`.`status` as 'status', `products`.`ico` as 'ico', `products`.`serial` as 'serial'",
			'locations' => "`locaionst`.`id` as 'id', `locaionst`.`town` as 'town', `locaionst`.`estate` as 'estate', `locaionst`.`point` as 'point'",
			'patners' => "`patners`.`id` as 'id', `patners`.`last_name` as 'last_name', `patners`.`first_name` as 'first_name', `patners`.`gender` as 'gender', if(`patners`.`birth_date`,date_format(`patners`.`birth_date`,'%m/%d/%Y'),'') as 'birth_date', `patners`.`age` as 'age', `patners`.`image` as 'image', `patners`.`city` as 'city', CONCAT_WS('-', LEFT(`patners`.`mobile`,3), MID(`patners`.`mobile`,4,3)) as 'mobile', `patners`.`product` as 'product', `patners`.`brand` as 'brand', DATE_FORMAT(`patners`.`filed`, '%c/%e/%Y %l:%i%p') as 'filed', DATE_FORMAT(`patners`.`last_modified`, '%c/%e/%Y %l:%i%p') as 'last_modified'",
			'orders' => "`orders`.`id` as 'id', `orders`.`product` as 'product', `orders`.`client` as 'client', `orders`.`mobile` as 'mobile', `orders`.`brand` as 'brand', `orders`.`quantity` as 'quantity', `orders`.`amount` as 'amount', `orders`.`price` as 'price', `orders`.`town` as 'town', `orders`.`estate` as 'estate', `orders`.`point` as 'point', `orders`.`status` as 'status'"
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('Treestate_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('Treestate_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['Treestate_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['Treestate_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Treestate</a>
			</div>
